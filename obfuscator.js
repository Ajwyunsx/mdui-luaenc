...
function compileAST(ast){
    ...
    function visit(node){if(!node)return -1;const chunk=compiler.currentChunk;
        switch(node.type){
            ...
            case 'StringLiteral':{let str;const raw=node.raw;if(raw.startsWith('[[')){str=raw.slice(2,-2);}else if(raw.startsWith('[=')){const eqCount=(raw.match(/^\[=*/)||[''])[0].length-1;str=raw.slice(eqCount+2,-(eqCount+2));}else if(node.value!==undefined&&node.value!==null){str=node.value;}else{str=raw.slice(1,-1);}const si=chunk.addConstant(str);const reg=chunk.allocRegister();chunk.emit(OPCODES.LOADK,reg,si);return reg;}
