...
function compileAST(ast){
    ...
    function visit(node){if(!node)return -1;const chunk=compiler.currentChunk;
        switch(node.type){
            ...
            case 'StringLiteral':{let str;if(node.value!==undefined&&node.value!==null){str=node.value;}else{const raw=node.raw;if(raw.startsWith('[[')){str=raw.slice(2,-2);}else if(raw.startsWith('[=')){const m=raw.match(/^\[=+/);const lvl=m?m[0].length-1:0;str=raw.slice(lvl+2,-(lvl+2));}else{str=raw.slice(1,-1).replace(/\\n/g,'\n').replace(/\\t/g,'\t').replace(/\\r/g,'\r').replace(/\\\\/g,'\\').replace(/\\"/g,'"').replace(/\\'/g,"'");}}const si=chunk.addConstant(str);const reg=chunk.allocRegister();chunk.emit(OPCODES.LOADK,reg,si);return reg;}
            ...
            case 'BooleanLiteral':{const reg=chunk.allocRegister();chunk.emit(OPCODES.LOADBOOL,reg,node.value?1:0,0);return reg;}
            case 'NilLiteral':{const reg=chunk.allocRegister();chunk.emit(OPCODES.LOADNIL,reg,0);return reg;}
            case 'LogicalExpression':{const dR=chunk.allocRegister();const lR=visit(node.left);chunk.emit(OPCODES.MOVE,dR,lR);if(node.operator==='and'){chunk.emit(OPCODES.TEST,dR,0,0);const jp=chunk.instructions.length;chunk.emit(OPCODES.JMP,0);const rR=visit(node.right);chunk.emit(OPCODES.MOVE,dR,rR);chunk.instructions[jp].a=chunk.instructions.length-jp-1;}else{chunk.emit(OPCODES.TEST,dR,0,1);const jp=chunk.instructions.length;chunk.emit(OPCODES.JMP,0);const rR=visit(node.right);chunk.emit(OPCODES.MOVE,dR,rR);chunk.instructions[jp].a=chunk.instructions.length-jp-1;}return dR;}
